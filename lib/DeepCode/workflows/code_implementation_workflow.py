...
class CodeImplementationWorkflow:
    ...
    def __init__(self, config_path: str = "mcp_agent.secrets.yaml"):
        ...
        self.last_openai_messages: Optional[List[Dict[str, Any]]] = None
        self.last_openai_response_message: Optional[Dict[str, Any]] = None
        self.last_openai_usage: Optional[Dict[str, Any]] = None
        self.coding_messages_log: List[Dict[str, Any]] = []
        self._last_logged_system_prompt: Optional[str] = None
    ...
    def _log_coding_message(
        self,
        entry: Dict[str, Any],
        usage: Optional[Dict[str, Any]] = None,
        extra_fields: Optional[Dict[str, Any]] = None,
    ) -> None:
        """Record a single coding message with optional metadata"""
        if entry is None:
            return

        logged_entry = copy.deepcopy(entry)

        # Merge extra_fields first
        if extra_fields:
            for key, value in extra_fields.items():
                logged_entry[key] = value

        # Merge usage tokens directly into logged_entry
        if usage and isinstance(usage, dict):
            for token_field in ("prompt_tokens", "completion_tokens", "total_tokens"):
                token_value = usage.get(token_field)
                if token_value is not None:
                    logged_entry[token_field] = token_value

        self.coding_messages_log.append(logged_entry)
    ...
    def _append_conversation_message(
        ...
        """Append to active conversation and log it simultaneously"""
        messages.append(entry)
        self._log_coding_message(entry, usage=usage, extra_fields=extra_fields)
    ...
    async def run_workflow(
        ...
        **llm_kwargs,
        ...
        Args:
            llm_kwargs: LLM parameters like max_tokens, temperature, timeout, etc.
                       All parameters from generation_config will be passed through
        """
        # Set the read tools configuration
        self.enable_read_tools = enable_read_tools
        # Store LLM kwargs from caller (no defaults added here)
        self.llm_kwargs = llm_kwargs
        # Reset cached OpenAI telemetry per run
        self.last_openai_messages = None
        self.last_openai_response_message = None
        self.last_openai_usage = None
        self.coding_messages_log = []
        self._last_logged_system_prompt = None
        ...
        try:
            ...
            if pure_code_mode:
                ...
                results["code_implementation"] = await self.implement_code_pure(
                    plan_content, target_directory, code_directory, **self.llm_kwargs
                )
            else:
                pass
            ...
            return {
                ...
                "coding_messages": copy.deepcopy(self.coding_messages_log),
            }
...
- Execute commands to actually create the file structure"""
    ...
    async def implement_code_pure(
        self, plan_content: str, target_directory: str, code_directory: str = None, **llm_kwargs
    ) -> str:
        ...
        try:
            ...
            messages = []
            self._log_coding_message({"role": "system", "content": system_message})
            self._last_logged_system_prompt = system_message
...
**Current Objective:** Begin implementation by analyzing the plan structure, examining the current project layout, and implementing the first foundation file according to the plan's priority order."""
            ...
            self._append_conversation_message(
                messages, {"role": "user", "content": implementation_message}
            )
    ...
    async def _pure_code_implementation_loop(
        ...
        **llm_kwargs,
        ...
        while iteration < max_iterations:
            ...
            if self._last_logged_system_prompt != current_system_message:
                self._log_coding_message(
                    {"role": "system", "content": current_system_message}
                )
                self._last_logged_system_prompt = current_system_message

            # Round logging removed

            # Call LLM
            response = await self._call_llm_with_tools(
                client, client_type, current_system_message, messages, tools, **llm_kwargs
            )
            ...
            # Ensure assistant entry includes tool_calls and usage tokens so the
            # exact dict stored in messages/coding_messages_log contains the metadata.
            tool_calls = response.get("tool_calls") or []
            usage_payload = response.get("usage") or {}
            # Merge usage token fields into assistant entry if present
            assistant_entry = {"role": "assistant", "content": response_content, "tool_calls": tool_calls}
            if isinstance(usage_payload, dict):
                for k in ("prompt_tokens", "completion_tokens", "total_tokens"):
                    if k in usage_payload and usage_payload[k] is not None:
                        assistant_entry[k] = usage_payload[k]

            # Append and log (we still pass usage and extra_fields for backward compatibility)
            self._append_conversation_message(
                messages,
                assistant_entry,
                usage=usage_payload,
            ...
            if response.get("tool_calls"):
                ...
                # Fallback: Extract code from markdown if no tool calls but code blocks exist
                self.logger.info("⚠️ No tool calls found, attempting to extract code from markdown...")
                self.logger.info(f"Response content preview: {response_content[:500]}...")
                import re
                
                # Regex to find code blocks with optional language identifier
                # Matches ```language\n code \n``` or ```language code ```
                # code_blocks = re.findall(r"```(?:\w+)?\s+(.*?)```", response_content, re.DOTALL)
                code_blocks = self.extract_solidity_code(response_content)
                
                tool_results = []
                if code_blocks:
                    # Assume the first block is the code we want to save
                    # We need to infer the filename. This is tricky without structured output.
                    # We'll try to find a filename in the text preceding the block or default to 'generated_code.sol'
                    # For now, let's try to parse the plan to see what file we should be working on?
                    # Or just use a timestamped filename if we can't find one.
                    
                    # Better approach: Look for "File: filename" pattern or similar in response
                    filename_match = re.search(r"(?:File|Filename):\s*([^\s\n]+)", response_content, re.IGNORECASE)
                    if filename_match:
                        filename = filename_match.group(1).strip()
                    else:
                        # Fallback to a generic name or try to extract from the code itself (e.g. contract Name)
                        filename = f"generated_code_{int(time.time())}.sol"
                    ...
                    if isinstance(code_blocks, list):
                        ...
                        # Not a list: assume a string (or fallback to str())
                        code_content = code_blocks.strip() if isinstance(code_blocks, str) else str(code_blocks)
                    
                    self.logger.info(f"📝 Extracted code for {filename}")
                    
                    # Simulate write_file tool call
                    simulated_tool_call = {
                    ...
                    tool_results = await code_agent.execute_tool_calls([simulated_tool_call])
                    
                    # Append a note to the message history so the model knows what happened
                    self._append_conversation_message(
                        messages,
                        {
                            "role": "user",
                            "content": f"System: Code extracted from markdown and saved to {filename}.",
                        },
                    )
                else:
                    self.logger.warning("⚠️ No code blocks found in markdown despite ``` presence.")
            
            if response.get("tool_calls") or ("```" in response_content and tool_results):
                # Continue with existing logic for tool results processing
                pass
            else:
                 # No tool calls and no code blocks
                 pass

            # Resume original indentation for the rest of the block
            if response.get("tool_calls") or ("```" in response_content and tool_results):
                ...
                self._append_conversation_message(
                    messages, {"role": "user", "content": compiled_response}
                )
                ...
                self._append_conversation_message(
                    messages, {"role": "user", "content": no_tools_guidance}
                )
    ...
    def extract_solidity_code(response: str) -> str:
        ...
        solidity_matches = re.findall(r"```solidity[:\s]*[^\n]*\n([\s\S]*?)```", code_block, re.MULTILINE)
        if solidity_matches:
        ...
        code_block = re.findall(r"```(?:\w+)?\s+(.*?)```", response, re.DOTALL)
    ...
    async def _initialize_llm_client(self):
        ...
        async def init_anthropic():
            ...
            try:
                ...
                openai_config = self.api_config.get("anthropic", {})
    ...
    async def _call_llm_with_tools(
        self, client, client_type, system_message, messages, tools, **llm_kwargs
    ):
        """Call LLM with tools"""
        try:
            if client_type == "anthropic":
                return await self._call_anthropic_with_tools(
                    client, system_message, messages, tools, **llm_kwargs
                )
    ...
    async def _call_anthropic_with_tools(
        self, client, system_message, messages, tools, **llm_kwargs
    ):
        """Call Anthropic API"""
        ...
        # Use kwargs from caller (no defaults here - set by run_workflow)
        anthropic_kwargs = llm_kwargs.copy()
        ...
        # Extract usage following response_utils pattern
        usage_data = None
        ...
        if usage is not None:
            ...
            # Only create dict if we have actual values
            if prompt_tokens is not None or completion_tokens is not None or total_tokens is not None:
                usage_data = {}
                if prompt_tokens is not None:
                    usage_data["prompt_tokens"] = int(prompt_tokens)
                if completion_tokens is not None:
                    usage_data["completion_tokens"] = int(completion_tokens)
                if total_tokens is not None:
                    usage_data["total_tokens"] = int(total_tokens)

        return {
            "content": content,
            "tool_calls": tool_calls,
            "openai_messages": self.last_openai_messages,
            "assistant_message": self.last_openai_response_message,
            "usage": usage_data,
        }
    ...
    async def _call_google_with_tools(
        ...
        # Map kwargs from caller (no defaults here - set by run_workflow)
        google_kwargs = {}
        # Map max_tokens to max_output_tokens for Google API
        if 'max_tokens' in llm_kwargs:
            google_kwargs['max_output_tokens'] = llm_kwargs['max_tokens']
        # Copy other compatible parameters
        for key in ['temperature', 'top_p', 'top_k', 'candidate_count']:
            if key in llm_kwargs:
                google_kwargs[key] = llm_kwargs[key]
        ...
        # Google path doesn't provide token usage in this implementation; return usage as None
        return {"content": content, "tool_calls": tool_calls, "usage": None}
    ...
    async def _call_openai_with_tools(
        ...
        # Use kwargs from caller (no defaults here - set by run_workflow)
        openai_kwargs = llm_kwargs.copy()
        # Prepare usage_data default so it's always available to the return path
        usage_data = None
            ...
            try:
                ...
                try:
                    ...
                    if "max_tokens" in error_str and "max_completion_tokens" in error_str:
                        ...
                        completion_kwargs = openai_kwargs.copy()
                    ...
                    elif "tool choice requires" in error_str or "'code': 400" in error_str:
                         # Fallback for Ollama/models that don't support auto tool choice
                        print(f"[WARNING] Tool choice error detected, retrying without tools: {e}")
                        response = await client.chat.completions.create(
                            model=self.default_models["openai"],
                            messages=openai_messages,
                            tools=None,
                ...
                request_messages_snapshot = copy.deepcopy(openai_messages)
                self.last_openai_messages = request_messages_snapshot
                self.last_openai_response_message = assistant_message
                self.last_openai_usage = usage_data
                ...
                if attempt < max_retries - 1:
                    ...
                    return {
                        ...
                        "tool_calls": [],
                        "openai_messages": copy.deepcopy(openai_messages),
        ...
        # Include usage for OpenAI so callers can attach token metadata
        return {"content": content, "tool_calls": tool_calls, "usage": usage_data}
