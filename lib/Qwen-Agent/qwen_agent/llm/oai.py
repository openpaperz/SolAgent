                ...
                for chunk in response:
                    if chunk.choices:
                        ...
                        if full_tool_calls:
                            res += full_tool_calls
                        yield res
                    # Capture usage info from the last chunk (vLLM support)
                    if hasattr(chunk, 'usage') and chunk.usage:
                        if res and res[0] and res[0].role == ASSISTANT and res[0].content:
                            res[0].extra={'prompt_tokens': chunk.usage.prompt_tokens, 'completion_tokens': chunk.usage.completion_tokens, 'total_tokens': chunk.usage.total_tokens}
                        ...
                        if (not prompt_tokens) and (not completion_tokens) and usage is not None:
                            ...
                            if isinstance(model_extra, dict):
                                ...
                                res[0].extra={'prompt_tokens': prompt_tokens, 'completion_tokens': completion_tokens, 'total_tokens': total_tokens}
                        yield res
