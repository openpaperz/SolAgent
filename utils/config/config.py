...
# Support both running as module and direct execution
try:
    from .constants import TOOL_PLUGIN_NAME
    from .env import Env
except ImportError:
    from constants import TOOL_PLUGIN_NAME
    from env import Env
                ...
                for name, value in _config.items():
                    if isinstance(value, BaseContainer):
                        ...
                        if key_match is not None:
                            print(f'[Config] Replacing {name} with extra value.')
                            setattr(_config, name, extra[key_match])
                        if (isinstance(value, str) and value.startswith('<')
                                and value.endswith('>')
                                and value[1:-1] in extra):
                            print(f'[Config] Replacing {value} with extra value.')
...
if __name__ == '__main__':
    ...
    # Add parent of utils to sys.path so imports work
    project_root = Path(__file__).resolve().parent.parent.parent
    if str(project_root) not in sys.path:
        sys.path.insert(0, str(project_root))
    
    # Re-import after fixing path (for direct execution)
    from utils.config.config import Config
    
    # Test loading coding.yaml
    code_config = Config.from_task("coding.yaml")
    print(f"[Test] Loaded config: {code_config.name}")
    print(f"[Test] LLM model: {getattr(code_config.llm, 'model', 'N/A')}")
    print(f"[Test] LLM service: {getattr(code_config.llm, 'service', 'N/A')}")
